    ...
    <div class="gate-way">
        <div class="third-header">
            <div class="banxin">
                ...
                <div class="sw" v-show="payVersion==2 ">
        ...
        <div class="body" v-show="payVersion==2 && type==0" :class="payVersion=='1'?'bd1':(active==1?'bd2_ac1':(active==2?'bd2_ac2':'bd2_ac3'))">
            <div class="banxin">
                ...
                <div class="c2"
                v-show="(payVersion=='2' && active!=3) ? true :false"
                :class="active==1?'c21 '+swA:(active==2?'c22 '+swA:(active==3?'c22 '+swA:'c22 '+swA))">
                    ...
                    <p class="c2two"> <i class="el-icon-time" v-show="active!=1"></i> <span v-show="active!=1">{{center_language[swA].nav_4}}：</span><span class="time" v-show="active!=1">{{fortmatTime(time)}}</span></p>
                </div>
                <!-- <div class="shangjia"
                ...
                >
                    ...
                    >   
                        <div class="llef">
                            <span class="span-p">{{center_language[swA].marketName}}：{{gate?gate.merchantName:""}}</span>
                            <div class="set">
                                ...
                                <div class="lleft_right">
                                    ...
                                    <p class="p_button">
                                        ...
                                        <button @click="createLegalorder()">{{center_language[swA].next}}</button>
                                    </p>
                                </div>
                            </div>
                        </div>
                    ...
                    >
                        <div class="box23_left">
                            <div class="bo">
                                <div class="to">
                                    <div class="border">
                                        <span>{{center_language[swA].num_cny}}：</span><span class="num">{{order==null?"":order.amountCny}}CNY</span>
                                    </div>
                                    <div class="info">
                                        <span>{{center_language[swA].info_1}} {{order?order.coinType.toUpperCase():''}} {{center_language[swA].info_2}}。</span>
                                    </div>
                                    <div class="border">
                                    ...
                                    <div class="one2_te">
                                        <span>{{center_language[swA].bank_user}}：</span><span>{{order==null?"":order.payOrderRealName}}</span>
                                    </div>
                                </div>
                                <div class="bot">
                            ...
                            <div class="bb">
                                <div class="to">
                                    <div class="one2">
                                        ...
                                        <div>
                                            <span>{{order==null?"":order.adReceiptWay.name}}</span>
                                            <span class="cp tag-read" :data-clipboard-text="order==null?'':order.adReceiptWay.name" @click="copyAddressFun()">{{center_language[swA].cp}}</span>
                                        </div>
                                    </div>
                                    <div class="one2">
                                        ...
                                        <div>
                                            <span>
                                                {{order==null?"":order.adReceiptWay.bankAccount}} {{order==null?"":order.adReceiptWay.branchAccount}}
                                            </span>
                                            <span class="cp tag-read" :data-clipboard-text="order==null?'':order.adReceiptWay.bankAccount+' '+(order==null?'':order.adReceiptWay.branchAccount)" @click="copyAddressFun()">{{center_language[swA].cp}}</span>
                                        </div>
                                    </div>
                                    <div class="one2">
                                        <span>{{center_language[swA].bank_num}}：</span>
                                        <div>
                                            <span>{{order==null?"":order.adReceiptWay.bankCardNumber}}</span>
                                            <span class="cp tag-read" :data-clipboard-text="order==null?'':order.adReceiptWay.bankCardNumber"
                                    ...
                                    <div class="one2" style="border:0px;">
                                        ...
                                        <div>
                                            ...
                                            :data-clipboard-text="order==null?'':order.referenceCode"
                                            @click="copyAddressFun()"
                                            >{{center_language[swA].cp}}</span>
                                        </div>
                                    </div>
                    ...
                    <div class="new_top_lef">
                        <div class="box_new">
                            <div class="box_one">
                                ...
                                <span class="color">{{order2?order2.amount:''}} {{order2?order2.coinType.toUpperCase():''}}</span>
                            </div>
                            <div class="box_te b">
                        ...
                        <div class="box_new">
                            <div class="box_one">
                                ...
                                <div class="box_one_set">
                                    <h3>{{order2?order2.receiptAddress:''}}</h3>
                                    <div class="ri">
                                        <span class="cp tag-read" :class="swA" :data-clipboard-text="order2?order2.receiptAddress:''" 
                                        @click="copyAddressFun()"
             ...
             <div class="new_body2" v-show="nac==2">
                 <li class="border">
                     ...
                     <div class="one_body2">
                         ...
                         <div class="r">
                             <span class="num">{{order2?order2.amount:''}} {{order2?order2.coinType.toUpperCase():''}}</span>
                         </div>
                     </div>
                 </li>
                 <li class="border">
                 ...
                 <li class="border">
                     ...
                     <div class="one_body2">
                         ...
                         <div class="r">
                             ...
                             <span class="button no" v-show="order3!=null && order3.status==0">
                                 {{new_language[swA].new_body2.confirm}}
                                 <i class="el-icon-loading" v-show="loadS"></i> 
                            </span>
                             <span class="button ok" v-show="order3!=null && order3.status==1">
        ...
        <p><span>{{new_language[swA].dia.one}}</span><span class="color">{{order2?order2.amount:''}} {{order2?order2.coinType.toUpperCase():''}}？</span></p>
    ...
    methods:{
        ...
        async createCoinOrder(item) {
            ...
            if(k){
                ...
                this.autoTime(k.expiredTimestamp,'time')
                // this.next1 = true;
        ...
        async createLegalorder(){
            ...
            item['amount']= this.gate.amount;
            item['amountCny'] = this.gate.amountCny;

            item['realName']=this.tName;
            let k = await createGateLegalOrder(this,item)
            if(k){
                this.order = k;
        ...
        getCode(){
            ...
            text: this.order2.receiptAddress,
            width: 155,
            height: 150,
            colorDark : '#000000',
            colorLight : '#ffffff',
        ...
        async alreadyPay(){
            ...
            let key =await alreadyPayConfirm(this,{
                id: this.order.id,
                receiptWayId: this.order.adReceiptWay.id
            }).then((res) => {
                return res;
            })
            if(key!=false) {
            ...
            .box2{
                ...
                .box2_p{
                    ...
                    .el-checkbox-group{
                        ...
                        .el-checkbox__inner{
                            background-color: rgb(51, 120, 97);
                            background: rgb(51, 120, 97);
                        }
                    }
                    span.bboc{
                        // display: block;
